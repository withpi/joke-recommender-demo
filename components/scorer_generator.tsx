...
import {SmallRadialLoader} from "@/components/radial_loader";
import {
...
import {PiIcon} from "@/components/pi_icon";
import {DarkArtifactButton} from "@/components/artifact_button";
import {GenerationLogsModal} from "@/components/generation_logs_modal";
import {GenerateScorerStatusIcon} from "@/components/generate_scorer_status_icon";
      ...
      <div className={'flex-1 pt-2 pb-3'}>
        <div
          className={clsx(
            ...
            isLast ? 'text-black' : 'text-zinc-600',
          )}
        >
          {status.message}
        </div>
        {status.systemMessages.length > 0 ? (
          <div
            className={clsx(
              ...
              status.systemMessages.length == 1
                ? 'pt-4'
                : status.systemMessages.length == 2
                  ? 'pt-8'
                  : 'pt-12',
            )}
          >
            <CardStack
              offset={15}
              scaleFactor={0.97}
              items={[...status.systemMessages].reverse()
                .map((m, i) => ({ ...m, id: `${m.message}-${i}` }))}
  ...
  return (
    <div className="flex animate-pulse items-stretch gap-2 text-sm">
      ...
      <div className={'flex-1 space-y-2 pt-2 pb-3'}>
        ...
        <div
          className={clsx('max-w-48 rounded-md bg-slate-200 p-1 font-semibold transition-colors')}
        />
        <div
          className={clsx('max-w-48 rounded-md bg-slate-200 p-1 font-semibold transition-colors')}
        />
...
function groupByContent(messages: GenerateScorerStatusMessage[]): GroupedSystemMessage[] {
  ...
  for (const generateScorerMessage of messages) {
    if (generateScorerMessage.target == 'user') {
      const lastMessage =
        aggregatedMessages.length > 0 ? aggregatedMessages[aggregatedMessages.length - 1] : null;
...
function GenerateScorerLoader({
  ...
  const openedLastMessage =
    ...
    aggregatedMessages[aggregatedMessages.length - 1].message == openedStatus?.message;
  console.log(job);
  return (
    <div className={'pt-2'}>
      {aggregatedMessages.length > 0 ? (
...
export function ScorerGenerator({
  ...
  if (!viewOnly || generatingScorer) {
    return (
      <div
        className={'relative mt-2 flex flex-col rounded-xl border bg-radial from-gray-100 to-white'}
      >
        <div className={'p-3'}>
          <Button
